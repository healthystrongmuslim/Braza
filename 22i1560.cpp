...
int main(){
    ...
    char** bord =new char* [x];
    for(i=0;i<x; bord[i++]=new char[x]);
...
void disp(char** A){
    system("clear");
    for(int i=0; i<x; cout<</*char(0*/i++<<endl)
        for(int j=0; j<x; cout<<"\33["<<(41+((A[i][j++]==' '))*3)<<"m   \33[0m");
    for(int i=0; i<x; cout<<((i>9)? " ":"  ")<<i++);
    cout<<endl;
}

char rondchkr(int i, int j, char** A){
...
void update(char** A){
    short i,j;
    char** bord =new char* [x];
    for(i=0;i<x; bord[i++]=new char[x]); //temporary array to store updated values, so original array doesn't change itself while checked
    for(i=0;i<x;i++)
