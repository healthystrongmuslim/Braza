...
// Generate a dataset (similar to the one provided to my class) via my randGraphGen.cxx
// But that dataset isn't very good for visualization, as nodes are randomly connected all over.
// Better dataset (nodes connected to others visually closer to them) in randCloseGen.cxx soon.
// The board-drawing/visualization was my own choice.
/*
-----------------------PROJECT STATEMENT (copypasted)----------------------------------------------
---------------------------------------------------------------------------------------------------
PART 1: SHORTEST PATH
This dataset contains information about connections (e.g., frequency of interaction, intensity, or
relevance) among multiple users in a social network. In this project, you need to find the most
significant connections in this social network. To do this, you have to create a graph of the given
dataset. In your graph, the Node represents a user and the Edges between them indicate their
connection. The weight on the edges shows the distance between two users. The shorter the
distance, the stronger the connection.
Your tasks are as follows:
1. Create an undirected graph from the file which has 3 columns: the first two columns
represent users and the third represents the weight of the link between them.
2. Implement Dijkstra algorithm or A* algorithm to find the shortest path from the graph
created in (1).
For A* algorithm, use the following heuristic function h(n): The number of direct
connections a user has.
For a node n, h(n) equals the total number of neighbours connected to n. Implement
efficiently by combining actual cost g(n) and the heuristic h(n).
Please note implementing with A* algorithm contains bonus points as it is a more efficient
method of finding the shortest path.
-----------------------------------------------------------------------------------------------------
PART 2: DYNAMIC PROGRAMMING | LONGEST CHAINS OF INCREASING INFLUENCE
In a social network, each user can influence their direct connections. The level of influence for
each user is given in the Influences file where the first column represents the user and the second
column represents the influence score of the user. You need to find the longest chain of influence
in the network such that the influence score of a user in the chain is strictly greater than the
previous user.
Task:
1. Implement a dynamic programming algorithm to find the longest increasing path in
terms of influence scores in the graph.
2. Return the maximum length of the chain and the sequence of users in the chain.
-------------------------------------------------------------------------------------------------------
*/
#include <print>
#include <termios.h>
...
#define LOLIMIT 100
#define p1file "./graph.txt" //file that has all the edges/connections stored
#define p2file "./infs.txt" //lists influence score of each user/node
#define lablfile "./labl.txt" //Names of all users
...
public:
  ...
  int heuheu(int id)
  {
    return lists[id].size; // Subtract 1 to exclude self-connection
  }
  void InspectNodes()
  {
    int id;
...
//Chain
  ...
  vector<int> traceAStarPath(int src, int dest, unordered_map<int, int> &cameFrom)
  {
  ...
  vector<int> astra(int src, int dest) // means star lol
  {
    ...
    openSet.emplace(heuheu(src), src);
    ...
    fs[src] = heuheu(src) + gs[src]; // f[n] = as yet best guess distance from src to dest thru n
    // Map to store the path
    unordered_map<int, int> cameFrom;
    int count = 0, nodesProcessed = 0;
    ...
    while (!openSet.empty())
    {
      ...
      if (curr == dest)//You have arrived at your destination. مع سلاما
      {
        ...
        auto tracedPath = traceAStarPath(src, dest, cameFrom);
      ...
      openSet.pop();
      // print("\e[0;46mcur={} curg={} curf={}\n\e[m",curr,gs[curr],currentfScore);
      Node *nodebor = lists[curr].head->next;
      while (nodebor)
      {
        int nayborID = nodebor->connNode;
        int topG = gs[curr] + nodebor->weight;
        // if(MAX==gs[nayborID]) print("\e[0;45mne={} neg=\e[91mINF\e[0;45m topg={}\t",nayborID,topG);else print("\e[0;45mne={} neg={} topg={}\t",nayborID,gs[nayborID],topG);
        if (topG < gs[nayborID])
        {
          cameFrom[nayborID] = curr;
          gs[nayborID] = topG;
          fs[nayborID] = topG + heuheu(nayborID);
          openSet.emplace(fs[nayborID], nayborID);
          // print("\e[0;94madded it neg={} nef={}",gs[nayborID],fs[nayborID]);
        }
        // print("\n\e[m");
        nodebor = nodebor->next;
      }
      if(count%500==0)printBoard(processed,src,dest,curr), plswait(0.02);
    }
    //print("\e[101mCOUNT={}\tNodes Visited:{}\e[m\n", count, nodesProcessed);
    delete[]  processed;
...
int main(int argc, char** argv){
  ...
  //if() print("Really{}…\n",((edgeC*2)==userC*(userC-1))?" clever":""),exit(100);
